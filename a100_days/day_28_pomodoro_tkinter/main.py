...
def count_down(count):
    minutes = count // 60
    seconds = count % 60
    canvas.itemconfig(timer_text, text=f"{0 if minutes < 10 else ''}{minutes}:{0 if seconds < 10 else ''}{seconds}")
    if count > 0:
        global timer
...
# Label Timer
title_lable = Label(text="Timer", font=(FONT_NAME, 50), bg=YELLOW, fg=GREEN)
